...
fn main() {
    ...
    // If `parse` returns an `Err` `Result` variant, the `expect` call will crash the game
    // and print the message. If `parse` can successfully convert the string to a number,
    // it will return the `Ok` variant of `Result`, and `expect` will return the number
